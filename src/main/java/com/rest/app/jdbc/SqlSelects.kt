...
import com.rest.app.dataBase.MySQLs.deleteEmployee
import com.rest.app.dataBase.MySQLs.selectEmployee
...
class SqlSelects {
    ...
    fun deleteEmployee(employeeId: Int) {
        DriverManager.getConnection(
                "jdbc:postgresql://localhost:5432/rest_staff", "postgres", "post@post23").use { connection ->
            connection.prepareStatement(deleteEmployee()).use { statement ->
    ...
    fun selectEmployee(employeeId: Int) {
        DriverManager.getConnection(
                "jdbc:postgresql://localhost:5432/rest_staff", "postgres", "post@post23").use { connection ->
            connection.prepareStatement(selectEmployee()).use { statement ->
                statement.setInt(1, employeeId) // employeeId - это (?) из запроса
